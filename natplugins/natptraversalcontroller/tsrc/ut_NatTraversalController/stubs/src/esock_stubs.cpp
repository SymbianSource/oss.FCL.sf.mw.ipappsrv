...
RSocket::RSocket()
	{
	iUseBaseClassSubSessionhandle = ETrue;
	}


// -----------------------------------------------------------------------------
// RSocket::ChangeHandling
// -----------------------------------------------------------------------------
//
void RSocket::ChangeHandling(TBool aValue)
	{
	iUseBaseClassSubSessionhandle = aValue;
	}

// -----------------------------------------------------------------------------
// RSocket::SubSessionHandle
// -----------------------------------------------------------------------------
//
TInt RSocket::SubSessionHandle() const
	{
	if (iUseBaseClassSubSessionhandle)
		{ 
		// Use default symbian version
		return RSubSessionBase::SubSessionHandle();
		}
	else
		{
		// Use own version
		return 1; // only purpose is to differ from 0
		}
	}
...
TInt RSocket::Info(TProtocolDesc& aProtocol)
	...
	// ESoSocketInfo = 37
	//return SendReceive(37 ,TIpcArgs(&protDesc));
