/*
* Copyright (c) 2006-2007 Nokia Corporation and/or its subsidiary(-ies).
* All rights reserved.
* This component and the accompanying materials are made available
* under the terms of "Eclipse Public License v1.0"
* which accompanies this distribution, and is available
* at the URL "http://www.eclipse.org/legal/epl-v10.html".
*
* Initial Contributors:
* Nokia Corporation - initial contribution.
*
* Contributors:
*
* Description:    implementation
*
*/



#include "tnatfwunsaftlstransportstatetlsconnecting.h"
#include "mnatfwunsaftlstransportstateowner.h"


// -----------------------------------------------------------------------------
// TNATFWUNSAFTlsTransportStateTlsConnecting::
//     TNATFWUNSAFTlsTransportStateTlsConnecting
// -----------------------------------------------------------------------------
//
TNATFWUNSAFTlsTransportStateTlsConnecting::
TNATFWUNSAFTlsTransportStateTlsConnecting(
    MNATFWUNSAFTlsTransportStateOwner& aOwner )
    : TNATFWUNSAFTlsTransportStateBase( aOwner )
    {
    }

// ----------------------------------------------------------------------------
// TNATFWUNSAFTlsTransportStateTlsConnecting::State
// From TNATFWUNSAFTlsTransportStateBase
// ----------------------------------------------------------------------------
//
TNATFWUNSAFTlsTransportStateBase::TState
    TNATFWUNSAFTlsTransportStateTlsConnecting::State() const
    {
    return ETlsConnecting;
    }

// -----------------------------------------------------------------------------
// TNATFWUNSAFTlsTransportStateTlsConnecting::StateChangedL
// From TNATFWUNSAFTlsTransportStateBase
// -----------------------------------------------------------------------------
//
void TNATFWUNSAFTlsTransportStateTlsConnecting::StateChangedL( TInt aStatus )
    {
    User::LeaveIfError( aStatus );

    iOwner.EnterStateL( TNATFWUNSAFTlsTransportStateBase::ETlsConnected );
    }

// End of File

