...
TUint32 CDataDepository::CreateNewTableKeyL( TUint aTableMask, TUint32 aFieldMask ) const
    ...
    if ( err == KErrNotFound )
        ...
        if ( keyCount == 0 )
            ...
            keys.SortUnsigned( );
            TUint32 maxKey = aTableMask ^ keys[ keys.Count() - 1 ];  
...
TUint32 CDataDepository::CreateNewServerKeyL( TUint aTableMask, TUint32 aFieldMask ) const
    {
    const TUint32 KSubKeyMask = 0xff0fffff;
    ...
    if ( err == KErrNotFound )
        ...
        if ( keyCount == 0 )
            ...
            {
            //remove setting keys related to same server
            for ( TInt i( 0 ); i < keyCount; i++ )
                {
                keys[i] &= KSubKeyMask;
                }

            for ( TInt j( 0 ); j < keys.Count() - 1; j++ )
                {
                for ( TInt i( j + 1 ); i < keys.Count(); )
                    {
                    if ( keys[ j ] == keys[ i ] )
                        {
                        keys.Remove( i );
